...
本文将地磁传感器的检测概率与杂波强度作为描述量测可靠性的两项关键统计量，其中前者反映地磁传感器检测能力的变化，后者反映环境杂波水平的变化。
...
需要指出的是，邻道干扰与双车并行在多地磁传感器系统中通过时所产生的量测在时空上具有较强的相似性，即两类场景均可能在两侧车道传感器上产生同步响应，仅依据空间分布与触发模式难以区分。但两类场景的本质差异在于其轨迹可解释性不同：邻道干扰场景中仅存在一辆真实车辆，双侧量测可由同一条轨迹统一解释；而双车并行场景中存在两辆相互独立的车辆，双侧量测必须由两条独立轨迹分别解释，单一轨迹在短时间内可能对双侧量测形成近似解释，但该一致性难以在时间序列上持续维持。因此，本文以轨迹可解释性作为判别依据，在原有单帧特征似然基础上进一步引入基于固定滞后窗口的解释能力项，并结合场景状态递推实现邻道干扰与双车并行两类场景的区分。

记第 $g$ 个地磁传感器对在第 $n$ 次扫描的场景状态为
\begin{equation}
	s_{g,n}\in\{\mathcal I_1,\mathcal I_2,\mathcal P\},
\end{equation}
其中 $\mathcal I_1$ 表示车道 1 为真实车辆、车道 2 为邻道干扰，$\mathcal I_2$ 表示车道 2 为真实车辆、车道 1 为邻道干扰，$\mathcal P$ 表示双车并行。
...
为刻画多步时序上的轨迹可解释性，引入长度为 $L$ 的局部观测序列
\begin{equation}
	\mathcal O_{g,n}^{(L)}
	=
	\left\{
...
\begin{equation}
	\mathcal E_{g,n}^{(1)}
	=
	\sum_{\ell=n-L+1}^{n}\varepsilon_{g,\ell}^{(1)},
\end{equation}
\begin{equation}
	\mathcal E_{g,n}^{(2)}
	=
	\sum_{\ell=n-L+1}^{n}\varepsilon_{g,\ell}^{(2)},
\end{equation}
其中，$\varepsilon_{g,\ell}^{(1)}$ 表示在第 $\ell$ 次扫描内，强制将双侧量测由同一条轨迹统一解释时的最小归一化残差；$\varepsilon_{g,\ell}^{(2)}$ 表示允许双侧量测分别由两条独立轨迹解释时的最小归一化残差。于是，$\mathcal E_{g,n}^{(1)}$ 可视为单轨迹模型在窗口 $\mathcal O_{g,n}^{(L)}$ 上的累计解释代价，$\mathcal E_{g,n}^{(2)}$ 则为双轨迹模型在同一窗口上的累计解释代价。
...
\begin{equation}
	...
	=
	\begin{cases}
		\exp\!\left(-\eta\,\mathcal E_{g,n}^{(1)}\right),
		& s\in\{\mathcal I_1,\mathcal I_2\},\\[4pt]
		\exp\!\left(-\eta\,\mathcal E_{g,n}^{(2)}\right),
		& s=\mathcal P,
	\end{cases}
\end{equation}
其中 $\eta>0$ 为尺度参数。该项的含义是：若当前窗口内的双侧量测能够被单轨迹稳定解释，则 $\mathcal E_{g,n}^{(1)}$ 较小，从而 $\mathcal I_1$ 或 $\mathcal I_2$ 更易获得较大似然；反之，若必须引入两条独立轨迹方能维持双侧量测的运动一致性，则 $\mathcal E_{g,n}^{(2)}$ 更小，从而 $\mathcal P$ 更易占优。

在此基础上，将原有单帧特征似然与轨迹可解释性因子结合，得到场景观测似然
\begin{equation}
...
由此，场景判别不再仅依赖当前扫描的时间差和磁峰值差异，而是同时利用固定滞后窗口内单轨迹模型与双轨迹模型的累计解释能力，在时间递推过程中持续累积有利证据。当双侧量测能够被单轨迹稳定解释时，邻道干扰状态后验将逐步占优；而当单轨迹解释误差随时间累积增大、双轨迹解释更为稳定时，并行状态后验将持续升高，从而实现对真实双车并行与邻道干扰场景的区分。
...
考虑到原始地磁事件流缺乏完整的车辆级真值轨迹、场景标签和连续漏检标注，而邻道干扰、双车并行、换道以及地磁传感器退化等困难工况在自然采集中出现频率较低，若直接以真实道路数据作为第三章的主要定量评测样本，难以对各模块的性能进行细粒度分析。因此，本章采用仿真数据作为主要评测数据，少量真实检测数据仅用于标定地磁事件的统计特性，真实道路环境下的验证结果统一放在第四章展开。

\noindent （1）仿真数据生成与参数设置。

首先，按照第二章给出的单向双车道部署模型构建仿真路段。仿真场景包含两条行车道、沿纵向均匀布设的成对检测地磁传感器，以及车辆在路段内的直行、并行和换道等基本运动过程。对每一辆仿真车辆，记录其纵向位置、速度、车道归属和换道发生时刻等真值信息，作为后续性能评估的参考基准。
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.4cm}p{2.3cm}p{3.0cm}p{5.0cm}}
		...
		单侧地磁传感器数 & $N_{\mathrm{node}}$ & 与第二章部署模型一致 & 与实际部署拓扑保持一致 \\
		地磁传感器纵向间距 & $d$ & 15 m & 相邻同侧地磁传感器的纵向间距 \\
		...
		时间戳噪声 & $\sigma_t$ & 0.1 s & 地磁传感器触发时间误差 \\
		磁峰值噪声 & $\sigma_M$ & 50 nT & 地磁峰值测量误差 \\
		正常检测概率 & $P_D^{(0)}$ & 0.95 & 正常地磁传感器的基准检测概率 \\
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.4cm}p{2.6cm}p{3.0cm}p{5.0cm}}
		...
		场景类型 & 关键参数 & 取值范围 & 说明 \\
		\midrule
		邻道干扰 & 邻道触发概率 $p_{\mathrm{adj}}$ & 0.10 & 对侧地磁传感器产生弱触发事件的概率 \\
		邻道干扰 & 峰值衰减系数 $\alpha_M$ & $0.1\sim0.2$ & 相对真实触发峰值的比例 \\
		双车并行 & 初始纵向间隔 $\delta_p$ & $0\sim5$ m & 保证两车在多个地磁传感器对上近似并行 \\
		双车并行 & 速度差 $\delta_v$ & $0\sim1.5$ m/s & 压缩两车相对触发时间差 \\
		换道场景 & 换道持续时间 $T_{\mathrm{lc}}$ & $2\sim4$ s & 横向状态平滑过渡，避免瞬时跳变 \\
		地磁传感器退化 & 退化检测概率 $P_D^{\mathrm{deg}}$ & $0.55\sim0.75$ & 用于地磁传感器后验实验 \\
		地磁传感器退化 & 杂波增强倍数 $\kappa_c$ & $2\sim4$ & 相对正常杂波强度的放大倍数 \\
...
为客观评估所提方法的有效性，本文设置四组对比算法，分别为仅采用单帧规则的基线方法、去除地磁传感器状态自适应的改写方法、去除场景递推的改写方法，以及本文完整方法。在消融实验中，进一步考察去除磁特征一致性、将软计数定稿更新替换为硬计数即时更新、以及去除固定滞后回放等关键模块对结果的影响。
...
\subsection{地磁传感器检测概率与杂波空间密度后验估计结果}

\noindent （1）仿真数据生成与参数设置。

为验证地磁传感器检测概率与杂波空间密度后验的估计能力，本文在表~\ref{tab:ch3_sim_global} 的全局参数基础上，对若干代表性地磁传感器施加正常、退化和恢复三段变化。在正常区间内，地磁传感器保持较高的检测概率和较低的杂波水平；在退化区间内，检测概率下降，杂波强度同步上升；在恢复区间内，两者重新回到正常水平。由于仿真过程中各地磁传感器的真实检测概率与杂波强度均为已知，因此可以直接将后验估计结果与真值进行比较。同时，进一步统计不同退化强度下的身份切换次数和误删轨迹次数，用于分析地磁传感器后验进入门控与关联过程之后对整体跟踪鲁棒性的影响。

\noindent （2）仿真结果分析。

图3.7用于验证地磁传感器检测特性的后验估计能力，
而图3.8进一步分析该检测特性建模对整体跟踪性能的影响，
两者分别从参数估计与跟踪效果两个层面验证所提方法的有效性。

图~\ref{fig:ch3_exp2_node_posterior} 给出了代表性退化地磁传感器的检测概率与杂波强度后验估计结果。本文所称地磁传感器自适应后验，是指对不同地磁传感器的检测概率与杂波强度进行在线自适应后验估计；相应地，地磁传感器固定参数后验不对上述两项统计量进行自适应更新，而是对所有地磁传感器采用统一固定参数。

\begin{figure}[H]
	\centering
	\includegraphics[width=0.88\textwidth]{code/chapter3/experiment1/exp1_results/3_7_exp1_node_posterior_curve.png}
	\caption{地磁传感器检测概率与杂波强度的后验估计结果}
	\label{fig:ch3_exp2_node_posterior}
\end{figure}

从图~\ref{fig:ch3_exp2_node_posterior} 可以看出，当地磁传感器进入退化区间后，本文采用的扫描冻结、软计数累积和窗口定稿后更新策略能够较快跟随真实检测概率的下降趋势，并在退化结束后重新收敛到稳定高值。相比之下，全局统一后验对局部退化的响应存在明显滞后，难以准确刻画单地磁传感器性能突变。对于杂波强度，自适应后验能够在退化区间内持续抬升，并在恢复阶段逐步回落，说明该机制能够同时感知漏检增多和误触发增多两类退化现象。

图~\ref{fig:ch3_exp1_idsw} 和图~\ref{fig:ch3_exp1_false_delete} 分别给出了地磁传感器检测能力下降程度与杂波强度变化对身份切换次数和误删轨迹次数的影响。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{code/chapter3/experiment1/exp1_results/3_8_1_exp1_degradation_impact.png}
	\caption{地磁传感器检测能力下降程度与杂波强度对身份切换次数的影响}
	\label{fig:ch3_exp1_idsw}
\end{figure}

图~\ref{fig:ch3_exp1_idsw} 给出了地磁传感器检测能力下降程度与杂波强度变化对身份切换次数的影响。可以看出，随着地磁传感器检测能力逐步下降、杂波强度持续增大，各方法的身份切换次数整体均呈上升趋势，但本文方法在不同退化与杂波组合下始终保持较低水平，且增长幅度相对更缓。这说明所提出的地磁传感器状态自适应后验能够较准确地表征地磁传感器可靠性的变化，并将其进一步作用于门控与关联决策，从而有效抑制由检测退化和杂波增强引起的错误匹配，降低轨迹身份发生切换的概率。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{code/chapter3/experiment1/exp1_results/3_8_2_exp1_degradation_impact.png}
	\caption{地磁传感器检测能力下降程度与杂波强度对误删轨迹次数的影响}
	\label{fig:ch3_exp1_false_delete}
\end{figure}

综合图~\ref{fig:ch3_exp2_node_posterior}、图~\ref{fig:ch3_exp1_idsw} 和图~\ref{fig:ch3_exp1_false_delete} 可以看出，地磁传感器状态自适应机制不仅改善了退化场景下的参数辨识能力，而且为后续量测关联、场景判别以及连续漏检容忍提供了更为可信的先验支持，是本章在线跟踪框架保持稳定性能的重要基础。
...
邻道干扰与真实双车并行是双车道地磁事件流中最容易混淆的两类场景，也是原有单帧阈值规则最容易失效的部分。为此，本文构造三类典型序列，分别对应单车正常通过、单车通过并伴随邻道干扰，以及两车长时间并行通过。对于困难场景，将两车的初始纵向间隔和速度差压缩在较小范围内，使其在连续多个地磁传感器对上保持近似并行，并有意将两车相对时间差长期控制在经验阈值附近，以突出单帧证据不足而多步证据递推有效这一差异。此外，在部分样本中以小概率注入换道行为，使车道后验能够在固定滞后窗口内完成平滑翻转，用于验证场景后验显式进入车道状态递推后的稳定性。

\noindent （2）仿真结果分析。

%为便于说明图3.9中不同方法的差异，表\ref{tab:method_compare_ch3}给出了各方法在多帧递推与检测特性建模方面的主要区别。
%
%\begin{tabular}{ccc}
%	\hline
%	方法名称 & 是否采用多帧递推 & 是否进行检测特性自适应 \\
%	\hline
%	本文方法 & 是 & 是 \\
%	单帧概率判别方法 & 否 & 否 \\
%	单帧阈值规则方法 & 否 & 否 \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.88\textwidth]{code/chapter3/experiment1/exp1_results/3_9_hard_parallel_tuned.png}
	\caption{困难双车并行场景下的场景后验递推过程}
	\label{fig:ch3_exp3_hard_case}
\end{figure}

图~\ref{fig:ch3_exp3_hard_case}给出了一个困难双车并行样本的场景后验递推过程。该样本的初始纵向间隔约为 $1.4\,\mathrm{m}$，速度差范围为 $0.07$--$1.30\,\mathrm{m/s}$，双侧相对时间差范围为 $1$--$56\,\mathrm{ms}$，未注入换道动作，连续经过 $18$ 对地磁传感器。可以看到，在该类临界并行情形下，单帧阈值规则方法在整个地磁传感器序列上几乎始终停留在邻道干扰状态，未能恢复真实双车并行关系；单帧概率判别方法虽然在部分地磁传感器对处能够给出正确判断，但其结果在相邻地磁传感器对之间仍存在明显波动，难以形成稳定场景结论。相比之下，本文方法在前期若干地磁传感器对上完成多步证据累积后，双车并行状态后验逐步占优，并在后续区间内保持较高稳定性。

该结果表明，当双侧相对时间差持续较小、地磁扰动峰值差异又不足以单独支撑硬判决时，仅依赖单次观测进行判别，容易将真实双车并行误判为邻道干扰，或在两种场景之间反复切换；而引入状态持续性约束与多步场景后验递推后，算法能够利用连续多个地磁传感器对上的弱判别信息逐步拉开不同场景的后验差异，从而显著缓解上述失效模式。
...
对于MHT和JPDA方法，其不直接输出场景状态。本文基于地磁传感器对尺度的双侧轨迹结果进行后处理映射：当同一地磁传感器对上仅存在一条可解释的连续轨迹时，判定为邻道干扰；当同一地磁传感器对上存在两条相互独立且满足运动一致性的轨迹时，判定为双车并行。对于存在歧义的情况，依据轨迹连续性与时间一致性进行判别。
...
本节用于验证扫描化处理、事件驱动更新和固定滞后回放相结合的时间组织方式是否合理。实验以实测延迟分布为约束，对事件到达顺序进行随机扰动，并分别扫描时间片长度 $\Delta T$ 和固定滞后窗口长度 $L$。在每组配置下，统计同一扫描内相邻地磁传感器的同车触发比例、迟到量测吸收比例，以及在线回放结果与按触发时间离线重排结果之间的一致率，用于考察时间组织参数对在线结果稳定性的影响。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{code/chapter3/experiment1/exp1_results/3_12_merged_geomag.png}
	\caption{时间组织参数与固定滞后窗口的收益与开销}
	\label{fig:ch3_exp5_time}
\end{figure}

首先，随着 $\Delta T$ 增大，同一扫描内相邻地磁传感器同车触发比例单调提升，说明过小的扫描间隔会削弱扫描组织对相邻地磁传感器事件的吸纳能力，而适度增大 $\Delta T$ 有利于提升局部时间组织的一致性。其次，迟到量测被窗口吸收的比例随 $L$ 增大而持续上升。当 $L=0,1,2,3,4,5$ 时，吸收比例分别为 $0.2970$、$0.3937$、$0.5250$、$0.6272$、$0.7194$ 和 $0.7824$。再次，离线重排与在线回放结果一致率也随 $L$ 增长而稳步提升，对应取值分别为 $0.4620$、$0.5121$、$0.5878$、$0.6482$、$0.7064$ 和 $0.7456$。这说明固定滞后窗口确实能够在不过度牺牲实时性的前提下吸收迟到量测，并显著改善在线处理与离线最优时间顺序之间的一致性。
...
为验证本章方法不是对原有规则的局部修补，而是由若干可解释模块共同构成的统一概率递推模型，本节从场景判别、跟踪连续性和时间回放三个角度分别进行定向消融，并进一步分析固定滞后窗口长度对额外开销的影响。具体消融项包括去除三状态场景递推、去除地磁传感器状态自适应、去除磁特征一致性、将软计数定稿更新替换为当前扫描的硬计数即时更新，以及去除固定滞后回放模块。
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		去除地磁传感器状态自适应 & 12.70 & 23.50 & 24.50 & 0.9188 \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{code/chapter3/experiment1/exp1_results/3_11_1_continuity.png}
	\caption{不同连续漏检长度下的轨迹连续保持率}
	\label{fig:ch3_exp4_continuity}
\end{figure}

由图~\ref{fig:ch3_exp4_continuity}可见，在不同车流条件下，随着连续漏检地磁传感器个数 $m$ 的增加，轨迹连续保持率整体呈下降趋势。其中，左图对应常规车流条件，右图对应低密度车流条件。本文方法在常规车流条件下对连续 2 至 3 个地磁传感器漏检仍保持较稳定的轨迹连续性。当 $m=2,3,4$ 时，轨迹连续保持率分别为 $0.9233$、$0.9321$ 和 $0.8981$。在低密度车流条件下，该方法对连续漏检具有更强的容忍能力。当 $m=2,3,4$ 时，轨迹连续保持率分别为 $0.9821$、$0.9821$ 和 $0.9188$，整体表现优于常规车流条件。

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{code/chapter3/experiment1/exp1_results/3_11_2_rmse.png}
	\caption{不同连续漏检长度下的跨段 RMSE}
	\label{fig:ch3_exp4_rmse}
\end{figure}

由图~\ref{fig:ch3_exp4_rmse}可见，随着连续漏检长度增加，跨段位置均方根误差逐步增大。其中，左图对应常规车流条件，右图对应低密度车流条件。在常规车流条件下，当 $m=2,3,4$ 时，对应跨段位置均方根误差分别为 $11.90\ \mathrm{m}$、$12.35\ \mathrm{m}$ 和 $15.10\ \mathrm{m}$；在低密度车流条件下，当 $m=2,3,4$ 时，对应跨段位置均方根误差分别为 $4.78\ \mathrm{m}$、$8.90\ \mathrm{m}$ 和 $9.95\ \mathrm{m}$。这表明在交通干扰较小的场景下，本文方法不仅能够保持更高的轨迹连续性，同时能够将跨段定位误差控制在更低水平。

上述结果说明，第三章方法的稳定容忍区间主要集中在常规车流下的连续 2 至 3 个地磁传感器，以及低密度条件下的连续 3 至 4 个地磁传感器。一旦超过该范围，门控范围扩张和身份歧义会共同导致轨迹碎片化明显加重。从章节衔接角度看，该实验也为第~\ref{chap:cedas_fragment}章的研究定位提供了直接依据。由于第三章方法已经能够在在线条件下处理短时漏检和短连漏检问题，因此第~\ref{chap:cedas_fragment}章更适合作为其下游片段级重构模块，重点处理超过固定滞后窗口和删除阈值之后形成的长时间间断片段，而不是重复解决第三章已能较好覆盖的短间断情形。

\subsection{本节小结}

本节围绕地磁传感器后验估计能力、邻道干扰与双车并行判别能力、连续漏检容忍范围、时间组织一致性以及各关键模块的独立贡献进行了系统验证。实验结果表明：地磁传感器状态自适应后验能够在地磁传感器退化与恢复过程中稳定跟踪检测概率和杂波强度变化；三状态场景递推显著降低了真实双车并行被误判为邻道干扰的比例，并有效抑制车道状态抖动；在常规车流条件下，本文方法可较稳定地容忍连续 2 至 3 个地磁传感器漏检，在低密度条件下该范围可扩展至连续 3 至 4 个地磁传感器；固定滞后回放能够明显提升迟到量测吸收比例以及在线处理结果与离线重排结果的一致率。消融实验进一步说明，地磁传感器状态自适应、三状态场景判别、磁特征一致性、软计数定稿更新以及固定滞后回放均对本章方法的最终性能具有独立而明确的贡献。综合来看，上述结果不仅验证了本文方法的精度与鲁棒性，也明确了其适用边界，并为第四章面向长缺测工况的片段级在线聚类与轨迹重构提供了合理的输入条件与触发依据。
...
\subsection{输入样本表示}
本节对输入样本形式进行统一规定，其目的在于明确本文方法与第三章在线跟踪模块之间的数据接口关系，并给出后续聚类算法的输入表达形式。通过对输入样本的规范化定义，为后续轨迹片段的组织与处理提供统一基础。
...
需要说明的是，在本文方法中，簇是样本进入聚类算法后的统一组织形式。无论输入为轨迹片段还是未关联的单点量测，在进入算法后均首先被初始化为簇，并纳入簇图结构进行管理。换言之，簇由样本构成，是样本在聚类过程中的抽象表示形式。通过将样本统一表示为簇，并在簇图结构上进行组织与更新，可以为后续的相似度计算、片段拼接及轨迹重构提供统一的处理基础。
...
第四章关注的重点并不是重新评价第三章的事件级跟踪性能，而是考察在连续漏检已经使上游结果发生碎片化之后，能否进一步恢复属于同一车辆的断裂轨迹片段。因此，本节首先说明实测数据的采集场景与组织方式，并给出本章实验所采用的数据来源、训练评估划分以及输入输出接口。

本章实验采用东乡西高速公路实测数据。该路段为单向双车道场景，有效监测长度约为 $1080\,\mathrm{m}$。沿道路两侧车道线共布设 $144$ 个地磁传感器，相邻地磁传感器间距为 $15\,\mathrm{m}$，同时设置路侧监控摄像机，用于对车辆实际通行过程进行人工核验和结果对照。东乡西路段的监测场景与数据采集方式如图~\ref{fig:ch4_dxx_monitor} 所示，相关部署配置见表~\ref{tab:ch4_data_config}。
...
实测系统以事件流形式接收各地磁传感器上报的车辆通过记录。每条原始记录至少包含触发时间 \texttt{date}、入队时间 \texttt{inQueueTime}、地磁传感器编号 \texttt{devSort}、峰值地磁扰动 \texttt{geomPeak}、车道标识 \texttt{lane} 以及持续时间 \texttt{timeLength} 等信息。需要说明的是，这些原始事件并不是第四章直接聚类的对象，而是先输入第三章在线跟踪模块，经过量测时序矫正、状态估计和数据关联后，形成轨迹片段及未关联单点量测，再作为第四章轨迹重构算法的输入。这样处理的目的，是在保留真实道路环境非理想性的同时，将第四章的评价重点明确限定在长缺测条件下的片段级重构能力上，而不是重复评价事件级跟踪精度。

从路段长期运行情况看，地磁传感器非上报并非偶发，而是具有一定连续性。表~\ref{tab:ch4_reporting_status} 给出了三个代表性观测时段的统计结果。可以看到，地磁传感器非上报率最高达到 $50.7\%$，最大连续非上报数量达到 $5$ 个地磁传感器。这表明在真实场景下，系统不仅会遇到随机单点漏检，还会出现跨多个相邻地磁传感器的连续缺测，从而导致上游轨迹被切分为多个片段，这也正是第四章需要重点解决的问题。

\begin{table}[htbp]
	\centering
	\caption{代表性观测时段的地磁传感器非上报情况}
	\label{tab:ch4_reporting_status}
	\begin{tabular}{lccc}
		\toprule
		统计日期 & 2024.12.15 & 2024.12.16 & 2025.02.27 \\
		\midrule
		地磁传感器非上报率 & 50.7\% & 31.9\% & 33.3\% \\
		最大连续非上报数量 & 5 & 5 & 4 \\
		\bottomrule
	\end{tabular}
\end{table}

为避免参数选择与性能评估相互耦合，本文先从实测数据中随机选取 $20\,\mathrm{min}$ 原始序列，并结合监控视频完成车辆标签人工核验，用于算法参数标定；在此基础上，再选取一段长度为 $25\,\mathrm{min}$ 的独立实测序列作为主评测集，该序列共包含 $256$ 辆车辆。后续各对比方法均基于相同的数据来源进行实验，以保证结果具有可比性。

在实验组织上，本章对比对象包括仅采用第三章上游样本输出而不进行后续重构的方法、传统在线聚类基线方法以及本文提出的片段级在线聚类与轨迹重构方法。为系统分析不同长缺测强度下方法的适用性与鲁棒性，后续实验将在上述实测数据基础上进一步构造不同总体漏检率 $\eta_{\mathrm{miss}}$ 与最大连续缺测长度 $L_{\mathrm{miss}}$ 的组合场景，用于模拟传感器长期运行后检测能力下降、连续漏检增多以及复杂交通工况共同作用下的典型失效模式。

评价指标方面，本文综合采用片段拼接精确率 $P_{\mathrm{link}}$、片段拼接召回率 $R_{\mathrm{link}}$、片段拼接 $F1$ 值 $F1_{\mathrm{link}}$、错并率 $R_{\mathrm{wm}}$、碎片消减量 $\mathrm{Frag}_{\mathrm{drop}}$、身份切换次数 $\mathrm{IDSW}$、重构召回率 $R_{\mathrm{rec}}$ 以及聚类一致性指标 $\mathrm{FMI}$ 等指标对方法性能进行评价。其中，$F1_{\mathrm{link}}$ 用于综合衡量片段重构质量，$R_{\mathrm{wm}}$ 用于刻画误拼接风险，$\mathrm{Frag}_{\mathrm{drop}}$ 与 $\mathrm{IDSW}$ 则用于反映重构后轨迹连续性与身份稳定性。

\subsection{评价指标}

第四章的任务并不是重新评价第三章的点级跟踪误差，而是考察在长缺测条件下，系统能否将已经碎片化的轨迹片段重新归并为属于同一车辆的完整轨迹。因此，本章评价指标的设计重点不再放在单时刻位置或速度估计误差上，而是放在三个方面：其一，断裂片段之间的连接是否正确；其二，不同车辆的片段是否被错误地拼接到一起；其三，重构后的整体聚类结构与真实轨迹分组是否一致。基于这一目标，本文采用片段拼接精确率 $P_{\mathrm{link}}$、片段拼接召回率 $R_{\mathrm{link}}$、片段拼接 $F1$ 值 $F1_{\mathrm{link}}$、错并率 $R_{\mathrm{wm}}$、碎片消减量 $\mathrm{Frag}_{\mathrm{drop}}$ 以及聚类一致性指标 $\mathrm{FMI}$ 作为本章的主要评价指标。

为便于定义上述指标，设第三章输出并进入第四章重构流程的样本集合为
\begin{equation}
	\mathcal{U}=\left\{\mathbf{u}_1,\mathbf{u}_2,\ldots,\mathbf{u}_M\right\},
\end{equation}
其中，$\mathbf{u}_m$ 可以是轨迹片段，也可以是未被上游吸收的单点量测。根据人工标注或参考轨迹，可进一步得到真实可拼接关系集合
\begin{equation}
	\mathcal{L}^{\mathrm{gt}},
\end{equation}
其中任意一对 $\left(\mathbf{u}_i,\mathbf{u}_j\right)\in\mathcal{L}^{\mathrm{gt}}$ 表示二者属于同一真实车辆，且在时间顺序上应当被连接为同一条轨迹。相应地，算法输出的重构结果可表示为预测拼接关系集合
\begin{equation}
	\mathcal{L}^{\mathrm{pred}}.
\end{equation}
据此，定义真正例、假正例和假反例如下：
\begin{equation}
...
\end{equation}
该指标表示算法给出的所有拼接关系中，有多大比例是真实正确的连接。若 $P_{\mathrm{link}}$ 较高，说明算法在进行片段归并时较为谨慎，能够较好地避免将不同车辆的片段误接到同一条轨迹中。
...
\end{equation}
该指标表示所有真实应被恢复的片段连接关系中，有多大比例被算法成功找回。与 $P_{\mathrm{link}}$ 相比，$R_{\mathrm{link}}$ 更关注重构的完整性，即算法是否能够尽可能恢复长缺测造成的真实断裂关系。

由于精确率和召回率分别从“拼得准不准”和“拼得全不全”两个角度刻画性能，单独观察其中任意一个指标都可能产生片面结论。因此，本文进一步采用二者的调和平均作为片段拼接主指标，即
\begin{equation}
	F1_{\mathrm{link}}
	=
	\frac{2P_{\mathrm{link}}R_{\mathrm{link}}}{P_{\mathrm{link}}+R_{\mathrm{link}}}.
\end{equation}
$F1_{\mathrm{link}}$ 综合反映了重构结果在正确性与完整性之间的平衡程度，因此在本章中将其作为衡量片段级轨迹重构效果的核心指标。

为了更直接地表征误拼接风险，本文同时定义错并率为
\begin{equation}
	R_{\mathrm{wm}}
	=
	\frac{N_{\mathrm{FP}}}{N_{\mathrm{TP}}+N_{\mathrm{FP}}}.
\end{equation}
从定义上看，$R_{\mathrm{wm}}$ 反映的是算法所有已执行拼接中，错误拼接所占的比例。该指标越小，说明算法越不容易把本不属于同一车辆的片段错误合并。需要指出的是，在上述定义下，$R_{\mathrm{wm}}$ 与 $P_{\mathrm{link}}$ 在数值上互为补充关系，但本文仍单独保留该指标，是因为相较于精确率的正向表达，错并率更能直接反映工程应用中最敏感的误拼接风险。

除连接关系本身外，本章还关心重构前后轨迹碎片化程度的变化。设重构前片段总数为 $\mathrm{Frag}_{\mathrm{before}}$，重构后输出轨迹数为 $\mathrm{Frag}_{\mathrm{after}}$，则碎片消减量定义为
\begin{equation}
	\mathrm{Frag}_{\mathrm{drop}}
	=
	\mathrm{Frag}_{\mathrm{before}}-\mathrm{Frag}_{\mathrm{after}}.
\end{equation}
该指标描述重构过程使碎片数量减少了多少，数值越大，说明原本被切分的短片段被更充分地归并为较长轨迹，系统的碎片化程度得到更明显缓解。需要强调的是，$\mathrm{Frag}_{\mathrm{drop}}$ 只能说明“片段被合并了多少”，并不能单独说明“合并得是否正确”，因此必须结合 $F1_{\mathrm{link}}$ 和 $R_{\mathrm{wm}}$ 一起分析，才能避免把过度合并误认为性能提升。

最后，为从全局结构层面评价重构结果与真实轨迹分组的一致性，本文采用 Fowlkes--Mallows Index（FMI）作为聚类一致性指标。设真实分组为
\begin{equation}
	\mathcal{C}
	=
...
\end{equation}
对于任意样本对 $\left(\mathbf{u}_i,\mathbf{u}_j\right)$，其中 $i<j$，可将其划分为四类：$a$ 表示在真实分组与预测分组中均被划入同一类的样本对数量；$b$ 表示在真实分组中属于同一类、但在预测分组中被分开的样本对数量；$c$ 表示在真实分组中属于不同类、但在预测分组中被错误归为同一类的样本对数量；$d$ 表示在真实分组与预测分组中均被划分为不同类的样本对数量。于是，FMI 定义为
\begin{equation}
	\mathrm{FMI}
	=
	\sqrt{
...
\end{equation}
该指标本质上刻画的是预测聚类结构与真实分组之间的一致程度，其值越接近 $1$，说明算法输出的整体轨迹分组越接近真实情况。与仅关注局部连接关系的 $P_{\mathrm{link}}$、$R_{\mathrm{link}}$ 和 $F1_{\mathrm{link}}$ 相比，$\mathrm{FMI}$ 更侧重于从全局角度评价最终聚类结构是否合理。

为便于后续结果分析，表~\ref{tab:ch4_metric_summary} 总结了本章各指标的主要作用及优化方向。
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{0.19\textwidth}p{0.56\textwidth}c}
		...
		$F1_{\mathrm{link}}$ & 综合平衡片段拼接精确率与召回率，是本章的主评价指标 & 越大越好 \\
...
综合来看，$P_{\mathrm{link}}$、$R_{\mathrm{link}}$、$F1_{\mathrm{link}}$ 和 $R_{\mathrm{wm}}$ 主要从局部片段连接关系出发评价重构质量，$\mathrm{Frag}_{\mathrm{drop}}$ 用于刻画碎片化程度是否得到实质改善，而 $\mathrm{FMI}$ 则从整体聚类结构层面对结果进行补充验证。后续实验分析中，本文将以 $F1_{\mathrm{link}}$ 作为主指标，并结合 $R_{\mathrm{wm}}$、$\mathrm{Frag}_{\mathrm{drop}}$ 和 $\mathrm{FMI}$ 对方法的有效性进行综合判断。
...
本节在上一节所定义评价指标的基础上，对第四章方法在主场景下的整体聚类重构效果进行分析。分析思路分为三个层次：首先，利用片段集在线聚类结果的可视化图，对算法是否能够将属于同一车辆的断裂片段归并到同一簇中进行直观展示；其次，通过重构前后轨迹长度分布的变化，进一步说明碎片化是否得到有效缓解；最后，在统一输入、统一处理顺序和统一评价指标下，将本文方法与不同基线方法进行对比，从整体上评价本文方法在轨迹片段重构任务中的有效性。
...
图~\ref{fig:4_2_typical_case} 给出了主场景下片段集在线聚类结果的可视化示意。为了便于观察聚类效果，图中将每个片段样本的归一化位置、车道信息和归一化时间作为三维坐标轴，不同颜色表示最终归属的不同轨迹簇。需要说明的是，该图展示的是片段按到达顺序依次进入聚类算法后形成的最终簇结构，因此图中的每一个簇都可以理解为一条重构后的候选车辆轨迹。

从图中可以看出，大多数片段能够沿车辆实际行驶方向在三维空间中形成连续且相互分离的簇结构，说明本文方法可以较好地将长缺测后被切断的短片段重新组织为较完整的轨迹。对于局部距离接近、时间相邻甚至存在一定交叠趋势的片段，图中仍然保持了较好的簇间区分度，没有出现大面积混簇现象。这说明本文在片段级在线聚类过程中引入的时间顺序约束、方向一致性约束以及地磁辅助信息，能够在仅依据位置邻近关系容易发生混淆的区域提供额外判别依据，从而降低误拼接概率。换言之，图~\ref{fig:4_2_typical_case} 从直观层面表明，本文方法并非简单地将空间上相近的片段机械合并，而是在保持轨迹连续性的同时兼顾了聚类边界的合理性。

仅凭聚类可视化结果，还不足以完整说明重构质量是否得到实质提升。为此，图~\ref{fig:4_3_length_distribution} 进一步给出了重构前后轨迹长度分布的变化情况。可以看到，重构前的轨迹长度主要集中在较短区间，说明第三章输出在长缺测影响下仍保留了较明显的碎片化特征；而经过第四章聚类重构后，长度分布整体向中长轨迹区间移动，短片段数量明显减少，中等长度和较长长度轨迹的占比有所提升。该现象表明，本文方法能够有效恢复原本属于同一车辆的断裂片段，从而减少“只包含少量节点通过记录的短轨迹”在最终输出中的比例。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.82\textwidth]{4_3_length_distribution.png}
	\caption{重构前后轨迹长度分布对比}
	\label{fig:4_3_length_distribution}
\end{figure}

进一步观察图~\ref{fig:4_3_length_distribution} 还可以发现，长度分布虽然整体向长轨迹方向移动，但并未出现大量异常超长轨迹的集中堆积。这一点同样具有重要意义。因为在轨迹重构任务中，若算法仅依靠过度合并来减少碎片数量，虽然表面上会使轨迹长度增加，但往往会同时带来明显的误拼接问题。本文方法在长度分布改善的同时，没有出现明显的异常长尾扩张，说明其性能提升并非来源于粗放式合并，而是在保持聚类结构合理性的前提下实现了碎片消减。

为了更系统地评价本文方法的总体性能，下面进一步给出不同方法在同一主场景下的对比结果。为保证对比公平性与结果可复现性，所有方法均使用第三章输出的同一批待重构片段作为输入，并保持一致的样本到达顺序进行在线处理，不引入额外的人工后处理步骤。主场景中，重构前的初始碎片数均为 178。对比方法包括以下三类：其一，“仅上游样本输出”表示直接将第三章输出作为最终结果，不进行任何跨缺测的片段重构；其二，CEDAS 基线表示将经典在线密度聚类思想改写为适用于片段流输入的增量版本，主要依靠半径阈值和簇衰减机制完成片段吸收与簇更新；其三，CluStream 基线表示将流式微簇聚类方法应用于本章片段集输入，通过在线维护簇摘要并依据特征距离进行归并。与这些通用在线聚类方法相比，本文方法进一步引入了面向轨迹重构任务的簇图结构、方向一致性约束、地磁辅助判别以及簇尾摘要更新机制，因此更适合处理长缺测场景下的断裂轨迹片段。
...
\begin{table}[htbp]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{lcccccc}
			...
			本文方法        & \textbf{0.9208} & \textbf{0.8416} & \textbf{0.8794} & \textbf{0.0792} & \textbf{104} & \textbf{0.8803} \\
			\bottomrule
	\end{tabular}}
\end{table}

由表~\ref{tab:4_2_overall_metrics} 可以首先看出，仅依赖上游样本输出时，系统并不会主动恢复跨缺测形成的断裂关系，因此无法形成有效的片段拼接结果，$P_{\mathrm{link}}$、$R_{\mathrm{link}}$、$F1_{\mathrm{link}}$ 以及 $\mathrm{FMI}$ 均接近于 0，碎片消减量 $\mathrm{Frag}_{\mathrm{drop}}$ 也为 0。这说明在长缺测已经导致轨迹碎片化的情况下，仅依赖第三章的输出结果并不足以恢复完整车辆轨迹，必须通过后续的片段级聚类重构机制进一步消除碎片化影响。

与“仅上游样本输出”相比，CEDAS 基线和 CluStream 基线都能够在一定程度上完成片段归并，说明将通用在线聚类方法直接作用于片段流，确实能够对轨迹碎片化问题起到一定缓解作用。然而，两种基线方法的性能差异也反映出一般流式聚类方法在本任务中的局限性。CEDAS 基线的 $P_{\mathrm{link}}$ 为 0.5217，而 $R_{\mathrm{link}}$ 仅为 0.1086，$F1_{\mathrm{link}}$ 只有 0.1798，说明它只能恢复很少一部分真实断裂关系。同时，其错并率 $R_{\mathrm{wm}}$ 高达 0.4783，表明在已经执行的拼接中，错误连接占比接近一半。造成这一结果的主要原因在于，CEDAS 更适合处理一般意义上的密度聚类问题，而对于具有明确时间顺序和轨迹连续性约束的片段流数据，仅依靠局部邻近性和簇衰减机制难以准确刻画“哪些片段应当被连接、哪些片段不应被连接”。

CluStream 基线的结果优于 CEDAS 基线，$F1_{\mathrm{link}}$ 提升至 0.4624，$\mathrm{Frag}_{\mathrm{drop}}$ 达到 91，说明微簇摘要机制能够比简单的在线密度聚类更有效地组织片段流数据。但从结果上看，CluStream 基线的 $R_{\mathrm{wm}}$ 仍然达到 0.3986，$\mathrm{FMI}$ 也只有 0.4753，这表明虽然它能够合并更多片段，但最终形成的聚类结构与真实车辆轨迹之间仍存在较明显偏差。其根本原因在于，通用微簇方法虽然能够保留片段的局部统计摘要，但并未针对轨迹重构问题显式建模片段间的运动方向连续性、轨迹尾部演化信息以及地磁模式差异，因此在时空上相近但不属于同一车辆的片段之间仍容易产生混淆。

相比之下，本文方法在各项核心指标上均取得了最优结果。其 $P_{\mathrm{link}}$ 达到 0.9208，说明算法给出的绝大多数片段连接关系都是真实有效的；$R_{\mathrm{link}}$ 达到 0.8416，表明大部分真实断裂关系都能够被成功恢复；综合指标 $F1_{\mathrm{link}}$ 达到 0.8794，明显高于两类基线方法。与此同时，本文方法的错并率 $R_{\mathrm{wm}}$ 仅为 0.0792，远低于 CEDAS 基线和 CluStream 基线，说明本文方法在恢复更多真实连接关系的同时，并没有以牺牲连接正确性为代价。碎片消减量 $\mathrm{Frag}_{\mathrm{drop}}$ 达到 104，也高于各基线方法，表明其在抑制碎片化方面同样具有更强能力。全局聚类一致性指标 $\mathrm{FMI}$ 达到 0.8803，进一步说明本文方法不仅在局部连接关系上更准确，在整体聚类结构上也更接近真实车辆分组。

若进一步与性能最好的基线方法 CluStream 基线进行比较，本文方法的 $F1_{\mathrm{link}}$ 由 0.4624 提升至 0.8794，绝对提升 0.4170；错并率 $R_{\mathrm{wm}}$ 由 0.3986 降至 0.0792，绝对下降 0.3194；碎片消减量由 91 提升至 104；$\mathrm{FMI}$ 由 0.4753 提升至 0.8803。这一结果说明，本文方法的优势并不只是“聚得更多”，更重要的是“聚得更准”。如果一种方法只是通过粗略合并来减少碎片数量，那么通常会表现为 $\mathrm{Frag}_{\mathrm{drop}}$ 提升而 $R_{\mathrm{wm}}$ 明显恶化，或者 $\mathrm{FMI}$ 无法同步提高；而本文方法在碎片消减量、错并率和全局一致性三个方面同时取得改善，说明其确实能够在保持聚类边界合理性的基础上恢复更多真实轨迹。

综合图~\ref{fig:4_2_typical_case}、图~\ref{fig:4_3_length_distribution} 和表~\ref{tab:4_2_overall_metrics} 可以看出，本文提出的片段级在线聚类方法能够有效缓解长缺测带来的轨迹碎片化问题。一方面，在线聚类可视化结果表明同一车辆的断裂片段能够在簇图结构中形成较为连续且清晰分离的聚类形态；另一方面，重构前后的长度分布变化与总体评价指标共同表明，本文方法不仅减少了碎片数量，而且在拼接准确性和全局结构一致性方面均优于通用在线聚类基线方法，从而验证了第四章所提出聚类重构策略的有效性。
...
从表~\ref{tab:4_4_ablation} 可见，去掉磁特征项后，$F1_{\mathrm{link}}$ 由 0.8724 显著下降至 0.7960，同时错并率 $R_{\mathrm{wm}}$ 由 0.0476 大幅上升至 0.1379，说明在方向相近的片段之间，地磁特征是抑制错误拼接的关键约束。相较之下，去掉磁特征归一化虽然没有造成如此剧烈的性能退化，但 $F1_{\mathrm{link}}$ 与 $R_{\mathrm{rec}}$ 仍明显下降，说明归一化处理对于减弱地磁传感器间系统偏差、提高片段间地磁可比性具有现实必要性。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.88\textwidth]{4_4_xiaorong.png}
	\caption{关键机制消融结果对比}
...
\subsection{不同漏检强度下的性能}

为保证本章实验与第三章结果之间具有直接可比性，本节仍采用第三章仿真部分使用的道路场景、车流生成参数以及地磁响应设置作为基础数据，不再重新构造一套独立样本流。具体而言，先按照第三章的仿真设置生成车辆通过事件序列，再在事件级注入不同强度的漏检扰动，由第三章在线跟踪模块输出轨迹片段及未完成关联的量测，最后将这些结果作为本章片段级在线聚类与轨迹重构算法的输入。这样处理的目的，是把第四章方法放在真实的上下游衔接位置上进行评价，即重点考察当第三章输出已经出现碎片化以后，本章方法能否进一步恢复长缺测造成的断裂轨迹。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.92\textwidth]{code/chapter4/experiment123/chapter4_results/exp1/4_7_duiibi.png}
	\caption{第三章方法与加入片段级聚类重构后的跨段RMSE对比}
	\label{fig:ch4_rmse_upstream_cluster}
\end{figure}

首先比较仅采用第三章方法与“第三章方法+片段级聚类重构”两种处理链路的结果。由图~\ref{fig:ch4_rmse_upstream_cluster}可知，当连续漏检个数较小时，两种方法的跨段RMSE差别并不明显，这说明第三章方法对短时漏检和短连漏检已经具有较好的处理能力，本章方法不会对其原有结果造成明显扰动。随着连续漏检个数继续增加，聚类重构带来的收益开始逐步显现。在常规车流密度下，当连续漏检个数达到5时，仅采用第三章方法的跨段RMSE上升到 $19.80\,\mathrm{m}$，而加入片段级聚类重构后下降到 $13.80\,\mathrm{m}$，降幅约为 $30.30\%$；在低密度车流条件下，对应误差由 $14.40\,\mathrm{m}$ 下降到 $10.10\,\mathrm{m}$，降幅约为 $29.86\%$。这说明第四章方法的作用重点并不是替代第三章对短间断的在线跟踪能力，而是在上游输出已经碎片化之后，继续利用片段之间的方向、时间和地磁一致性完成后验重构，从而把系统的有效恢复范围进一步扩展到更长的连续漏检区间。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.82\textwidth]{code/chapter4/experiment123/chapter4_results/exp1/4_5_lianxvquece.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.82\textwidth]{code/chapter4/experiment123/chapter4_results/exp1/4_6_loujianlv.png}
	\caption{片段拼接$F1$值随总体漏检率变化}
	\label{fig:ch4_f1_vs_missrate}
\end{figure}

在验证与上游方法串联后的收益之后，进一步将本文方法与两种典型在线聚类算法 CEDAS 和 CluStream 进行比较。图~\ref{fig:ch4_f1_vs_lmiss}和图~\ref{fig:ch4_f1_vs_missrate}分别给出了片段拼接 $F1$ 值随最大连续缺测长度 $L_{\mathrm{miss}}$ 和总体漏检率 $\eta_{\mathrm{miss}}$ 变化的结果。总体来看，三种方法的性能排序始终保持为“本文方法 $>$ CluStream $>$ CEDAS”。其中，本文方法的 $F1_{\mathrm{link}}$ 在不同缺测强度下始终保持在较高水平，曲线变化较为平缓；CluStream 随着缺测增强而持续下降，在较强缺测条件下已经较难维持稳定拼接；CEDAS 的退化最为明显，在两类扫描条件下均很快降至较低水平。以最大连续缺测长度为例，当 $L_{\mathrm{miss}}=6$ 时，本文方法的 $F1_{\mathrm{link}}$ 仍保持在 $0.880$ 左右，而 CluStream 和 CEDAS 分别下降到 $0.462$ 和 $0.181$；以总体漏检率为例，当 $\eta_{\mathrm{miss}}=0.6$ 时，本文方法的 $F1_{\mathrm{link}}$ 仍约为 $0.880$，而 CluStream 和 CEDAS 分别仅为 $0.462$ 和 $0.182$。

造成上述差异的原因在于，CEDAS 和 CluStream 主要依据样本在特征空间中的邻近关系进行在线聚类，对“两个断裂片段是否属于同一车辆的连续延拓”缺乏专门建模；相比之下，本文方法面向片段级重构任务，在聚类过程中同时引入了方向约束、时间连续性、地磁代表值以及簇尾摘要更新等信息，因此在长缺测导致的候选冲突和身份歧义增大时，仍能较稳定地区分真实后继片段与干扰片段。进一步地，从图~\ref{fig:ch4_f1_vs_lmiss}和图~\ref{fig:ch4_f1_vs_missrate}还可以看出，本文方法对“漏检总量增加”和“单次连续断裂变长”两类退化来源都具有较强适应性，说明其并不是只在轻度缺测场景下有效，而是在中高强度缺测条件下仍能维持较好的片段拼接能力。


综合上述结果可以看出，第三章方法主要负责在事件级维持短时连续性，而本文提出的片段级在线聚类与轨迹重构进一步提升了系统对长缺测片段的恢复能力；同时，与 CEDAS 和 CluStream 相比，本文方法在不同漏检强度下都表现出更好的稳定性和更强的鲁棒性。两者结合后，可以形成从事件级在线跟踪到片段级后验重构的完整处理链路，更符合多地磁传感器长期运行条件下轨迹恢复的实际需求。
...
本文围绕单向双车道高速公路场景下的多地磁传感器车辆目标跟踪问题开展研究，针对量测乱序、短时漏检、噪声干扰、邻道干扰与双车并行易混淆，以及长期运行后传感器退化引起的长缺测和轨迹碎片化等问题，构建了“事件级在线跟踪—片段级在线重构”的两级处理框架。本文的主要创新在于，将固定滞后回放、地磁传感器状态自适应和场景概率判别统一到事件级在线跟踪过程中，并进一步提出面向长缺测的片段级在线聚类与轨迹重构方法，使常态工况跟踪与长缺测修复能够前后衔接。

在在线跟踪层面，本文提出了基于地磁传感器状态自适应与场景概率判别的多车辆跟踪方法。该方法通过时间窗口缓存和固定滞后回放处理迟到量测，在事件级状态估计的基础上，将地磁传感器检测概率、杂波强度与场景后验共同引入数据关联和轨迹管理过程，从而提升了复杂工况下的关联稳定性和车道判别能力。实验结果表明，在困难并行场景下，场景判别准确率提升至 $0.9262$；在常规车流条件下，系统可较稳定地容忍连续 $2\sim3$ 个地磁传感器漏检，在低密度条件下可扩展至连续 $3\sim4$ 个；当固定滞后窗口长度取 $L=4$ 时，迟到量测吸收比例达到 $0.7194$，在线与离线结果一致率达到 $0.7064$。这说明所提方法能够较好适应双车道高速场景下多地磁事件流的在线处理需求。
...
此外，虽然本文已经对地磁传感器检测概率和杂波强度进行了在线自适应建模，但固定滞后窗口长度、门控阈值和片段聚类权重等参数仍存在一定经验依赖；同时，当前对地磁信息的利用仍以峰值等低维统计特征为主，对波形细节和多尺度特征的挖掘还不够充分。后续可进一步研究参数自整定方法以及更丰富的地磁特征表示，并将其与车辆运动状态、车道信息联合建模，以提升复杂干扰和高相似目标条件下的区分能力。
