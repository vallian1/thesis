...
设第三章上游在线跟踪器按时间输出的第 $j$ 个输入样本记为 $\mathbf{u}_j$，则本文统一将其表示为
\begin{equation}
	...
	\Big),
\end{equation}
其中，$t_j^{\mathrm{s}}$ 和 $t_j^{\mathrm{e}}$ 分别表示样本的起始时刻和结束时刻；$\hat{\mathbf{x}}_j^{\mathrm{s}}$ 和 $\hat{\mathbf{x}}_j^{\mathrm{e}}$ 分别表示样本起点和终点的车辆运动状态估计；$\mathbf{P}_j^{\mathrm{s}}$ 和 $\mathbf{P}_j^{\mathrm{e}}$ 为对应状态估计的误差协方差；$\boldsymbol{\pi}_j$ 为样本的车道概率向量；$\bar m_j$ 和 $\sigma_{m,j}^{2}$ 分别表示样本磁场扰动峰值的统计均值与方差；$n_j$ 为样本所包含的原始观测数，用于表征样本支持度。

为便于后续统一处理，本文不再区分“轨迹片段”和“单点量测”两类输入对象，而是统一将其抽象为输入样本 $\mathbf{u}_j$。当输入来自轨迹片段时，上述字段由片段边界状态及相应统计量直接给出；当输入来自未被上游吸收的单点量测时，可将其视为长度退化为 1 的样本，此时有
\begin{equation}
...
\begin{equation}
	...
	\end{bmatrix},
\end{equation}
其中 $\hat p_j^{\mathrm{s}}$、$\hat p_j^{\mathrm{e}}$ 分别表示样本边界的纵向位置估计，$\hat v_j^{\mathrm{s}}$、$\hat v_j^{\mathrm{e}}$ 分别表示对应的纵向速度估计。相应的协方差矩阵 $\mathbf{P}_j^{\mathrm{s}}$ 与 $\mathbf{P}_j^{\mathrm{e}}$ 则刻画了样本边界状态估计的不确定性。
...
此外，$\bar m_j$ 与 $\sigma_{m,j}^{2}$ 分别表示样本对应磁场扰动峰值的均值和方差，用于在后续聚类过程中提供辅助判别信息；而 $n_j$ 则反映了该样本由多少原始观测支持形成，一般而言，$n_j$ 越大，说明该样本越稳定、越值得信任。
...
\end{equation}
其中，$\mathcal{G}_i$ 表示第 $i$ 个簇的图结构。本文将簇图结构定义为
\begin{equation}
...
对于任意样本 $\mathbf{u}_j$，若其被归并到簇 $\mathcal{G}_i$，则在簇图结构中对应一个节点 $v_j\in\mathcal{V}_i$。簇中的有向边用于表示样本之间在时间与特征空间上的可连接关系。需要说明的是，本文所称的\textbf{方向约束}，并非仅表示图中边的有向性，也不是单纯的几何方向信息，而是指样本在特征空间中沿车辆真实运动演化方向形成的可连接关系，其本质对应于车辆目标在时间、位置与速度等属性上的运动一致性约束。因此，若两个样本满足时间先后关系，并且在状态传播意义下符合车辆连续运动规律，则可在图结构中建立从前一节点指向后一节点的有向边。
...
\begin{equation}
	...
	\Big),
\end{equation}
其中，$t_i^{\mathrm{birth}}$ 表示簇的生成时刻，$t_i^{\mathrm{tail}}$ 表示当前尾端样本的结束时刻，$\hat{\mathbf{x}}_i^{\mathrm{tail}}$ 与 $\mathbf{P}_i^{\mathrm{tail}}$ 分别表示簇尾端状态估计及其协方差，$\boldsymbol{\pi}_i$ 为簇级车道概率向量，$\bar m_i$ 与 $\sigma_{m,i}^{2}$ 为簇级磁峰值统计量，$E_i$ 为簇活跃度，$s_i$ 为簇生命周期状态，$N_i$ 为簇累计支持度。
...
在完成输入样本与簇图结构定义之后，第四章的在线聚类与轨迹重构过程可以概括为以下步骤：首先，接收第三章输出的输入样本 $\mathbf{u}_j$；其次，依据当前活跃簇集合筛选候选簇，并利用方向约束对不符合运动一致性的候选关系进行剔除；随后，综合运动信息、磁特征相似性和车道一致性计算聚类相似度评分；再次，根据评分结果决定样本应归入已有簇还是初始化为新簇；最后，在样本归簇后同步更新簇图结构中的尾端状态、车道概率、磁峰值统计以及生命周期信息，并输出当前重构结果。
...
当新到达样本 $\mathbf{u}_j$ 无法与任何现有活跃簇形成足够高的聚类相似度评分时，系统将以该样本为起点初始化一个新的簇图结构。设新簇记为 $\mathcal{G}_{\mathrm{new}}$，则其节点集合和边集合分别初始化为
\begin{equation}
...
设当前新到达样本为 $\mathbf{u}_j$，候选簇为 $\mathcal{G}_i$，则首先定义二者之间的时间间隔与位置间隔：
\begin{equation}
	\Delta t_{ij}=t_j^{\mathrm{s}}-t_i^{\mathrm{tail}}, \qquad
	\Delta p_{ij}=\hat p_j^{\mathrm{s}}-\hat p_i^{\mathrm{tail}}.
\end{equation}
...
\begin{equation}
	...
	\;
	0<\Delta p_{ij}\le v_{\max}\Delta t_{ij}+d_0
	\right),
\end{equation}
其中，$T_{\max}$ 为最大允许拼接时间间隔，$v_{\max}$ 为经验速度上界，$d_0$ 为松弛项。若 $g_{ij}^{\mathrm{dir}}=0$，则说明样本与簇在方向约束意义下不具备可连接性，应直接剔除。由此可见，方向约束本质上就是对车辆运动一致性的先验限制，其作用是先从时序和运动方向上排除明显不合理的拼接关系。
...
除运动信息外，本文还引入磁特征相似性与车道一致性作为辅助约束。设簇 $\mathcal{G}_i$ 当前维护的磁峰值统计量为 $\bar m_i$ 和 $\sigma_{m,i}^{2}$，则样本与簇之间的磁特征相似度定义为
\begin{equation}
	S_{ij}^{\mathrm{mag}}
	=
	\exp\!\left(
	-\frac{
		...
		2\left(\sigma_{m,j}^{2}+\sigma_{m,i}^{2}+\varepsilon_m\right)
	}
	\right),
\end{equation}
其中 $\varepsilon_m$ 为稳定项。对于车道一致性，设簇车道概率向量为 $\boldsymbol{\pi}_i$，样本车道概率向量为 $\boldsymbol{\pi}_j$，则其相似度可写为
\begin{equation}
	S_{ij}^{\mathrm{lane}}
	=
	\boldsymbol{\pi}_i^{\top}\boldsymbol{\pi}_j.
\end{equation}

综合方向约束、运动一致性、磁特征相似性与车道一致性，定义样本 $\mathbf{u}_j$ 与簇 $\mathcal{G}_i$ 之间的聚类相似度评分为
\begin{equation}
	S_{ij}^{\mathrm{clus}}
	=
	g_{ij}^{\mathrm{dir}}
...
当样本 $\mathbf{u}_j$ 被归入已有簇 $\mathcal{G}_i$ 后，需要同时完成簇图结构的连接关系更新、尾端状态更新以及生命周期信息维护。
...
\begin{equation}
	...
	=
	\mathbf{F}(\Delta t_{ij}^{\mathrm{e}})\hat{\mathbf{x}}_i^{\mathrm{tail}},
\end{equation}
\begin{equation}
	\tilde{\mathbf{P}}_{i\rightarrow j}^{\mathrm{e}}
	=
	\mathbf{F}(\Delta t_{ij}^{\mathrm{e}})\mathbf{P}_i^{\mathrm{tail}}\mathbf{F}(\Delta t_{ij}^{\mathrm{e}})^{\top}
	+
...
\begin{equation}
	\boldsymbol{\pi}_i^{+}
	=
	\frac{N_i\boldsymbol{\pi}_i+n_j\boldsymbol{\pi}_j}{N_i+n_j},
	\qquad
	\boldsymbol{\pi}_i
	\leftarrow
...
最后，在生命周期管理方面，本文为每个簇维护活跃度 $E_i$ 及生命周期状态 $s_i$。当簇成功吸收新样本时，其活跃度得到增强；当簇在一段时间内未再被更新时，其活跃度随时间衰减。设当前处理时刻为 $t$，则簇活跃度的时间衰减可表示为
\begin{equation}
	E_i(t)=E_i(t_i^{\mathrm{tail}})\exp\!\left(-\frac{t-t_i^{\mathrm{tail}}}{\tau_E}\right),
\end{equation}
其中 $\tau_E$ 为能量时间常数。当样本 $\mathbf{u}_j$ 被并入簇 $\mathcal{G}_i$ 后，执行
\begin{equation}
	E_i \leftarrow \min\!\left(1,\ E_i+\kappa_E\cdot \min\!\left(1,\frac{n_j}{n_{\mathrm{ref}}}\right)\right),
	\qquad
	s_i \leftarrow \mathrm{active}.
\end{equation}
反之，若簇在连续一段时间内未能吸收新样本，且满足
\begin{equation}
	E_i < E_{\min}
	\quad \text{且} \quad
	t-t_i^{\mathrm{tail}} > T_{\mathrm{del}},
\end{equation}
