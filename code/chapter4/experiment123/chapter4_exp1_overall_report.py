...
def plot_fig_4_3_length_distribution() -> None:
    ...
    ax.hist(frag_lengths, bins=bins, density=True, alpha=0.65, label="上游输出片段长度")
    ax.hist(recon_lengths, bins=bins, density=True, alpha=0.65, label="重构后轨迹长度")
    ax.set_title("重构前后轨迹长度分布")
    ax.set_xlabel("轨迹长度（节点数）")
    ax.set_ylabel("概率密度")
