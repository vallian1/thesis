...
def plot_exp3_hard_case(hard_case, out_path):
    ...
    axes[0].set_title('Exp.3 hard parallel case: scene posterior recursion')
...
def plot_exp3_metrics(agg_df, out_path):
    ...
    labels = ['single-frame rule', 'multi-step recursion']
    axes[0].bar(labels, [agg_df['rule_acc'], agg_df['full_acc']])
    axes[0].set_title('scene accuracy')
    axes[0].grid(axis='y', alpha=0.3)
    axes[1].bar(labels, [agg_df['rule_parallel_as_adj'], agg_df['full_parallel_as_adj']])
    axes[1].set_title('P -> adjacent error ratio')
    axes[1].grid(axis='y', alpha=0.3)
    axes[2].bar(labels, [agg_df['rule_jitter'], agg_df['full_jitter']])
    axes[2].set_title('lane-state jitter per sequence')
    axes[2].grid(axis='y', alpha=0.3)
    fig.suptitle('Exp.3 aggregate metrics')
    plt.tight_layout()
    plt.savefig(out_path, dpi=220, bbox_inches='tight')
    plt.close(fig)
...
def plot_exp4_miss_tolerance(df, out_path):
    ...
    for density, ax in zip(['normal', 'low'], axes):
        ...
        for method in ['rule', 'global', 'full']:
            ...
            ax.plot(ss['miss_len'], ss['survival'], marker='o', linewidth=2, label=f'{method}-survival')
            ax.plot(ss['miss_len'], ss['RMSE_p'], marker='s', linewidth=1.5, linestyle='--', label=f'{method}-RMSEp')
        ax.set_title(f'{density} density')
        ax.set_xlabel('consecutive missed beacons m')
        ax.grid(alpha=0.3)
    axes[0].set_ylabel('survival rate / bridge RMSE (m)')
    axes[0].legend(fontsize=8, ncol=2)
    fig.suptitle('Exp.4 miss tolerance vs consecutive missing length')
    plt.tight_layout()
    plt.savefig(out_path, dpi=220, bbox_inches='tight')
    plt.close(fig)
...
def plot_exp5_time_consistency(scan_df, lag_df, out_path):
    ...
    axes[0].set_title('same-scan adjacent-node ratio')
    axes[0].set_xlabel('ΔT (s)')
    axes[0].grid(alpha=0.3)

    ll = lag_df.sort_values('L')
    axes[1].plot(ll['L'], ll['late_absorb_ratio'], marker='o', linewidth=2)
    axes[1].set_title('late-measurement absorption')
    axes[1].set_xlabel('lag window L (scans)')
    axes[1].grid(alpha=0.3)

    axes[2].plot(ll['L'], ll['consistency'], marker='o', linewidth=2)
    axes[2].set_title('offline/online result consistency')
    axes[2].set_xlabel('lag window L (scans)')
...
def plot_ablation(ab_df, out_path):
    ...
    axes[0].bar(x - 1.5 * width, ab_df['RMSE_p'], width, label='RMSEp')
    axes[0].bar(x - 0.5 * width, ab_df['IDSW'], width, label='IDSW')
    axes[0].bar(x + 0.5 * width, ab_df['Frag'], width, label='Frag')
    axes[0].bar(x + 1.5 * width, ab_df['bridge_survival'] * 100.0, width, label='Survival x100')
    axes[0].set_title('Ablation: tracking-side metrics')
    axes[0].grid(axis='y', alpha=0.3)
    axes[0].legend(ncol=4)

    axes[1].bar(x - 0.20, ab_df['scene_acc'], 0.40, label='scene accuracy')
    axes[1].bar(x + 0.20, ab_df['lane_acc'], 0.40, label='lane accuracy')
    axes[1].set_title('Ablation: scene / lane metrics')
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(variants, rotation=15)
    axes[1].grid(axis='y', alpha=0.3)
...
def plot_complexity(lag_df, out_path):
    ...
    ax1.set_xlabel('lag window L (scans)')
    ax1.set_ylabel('avg buffered events')
    ax1.grid(alpha=0.3)
    ax2 = ax1.twinx()
    ...
    plt.title('Complexity/benefit trade-off for replay window L')
    plt.tight_layout()
    plt.savefig(out_path, dpi=220, bbox_inches='tight')
    plt.close(fig)
...
def main():
    base_dir = '/mnt/data/chapter3_exp345_results'
    os.makedirs(base_dir, exist_ok=True)
    bank = load_real_bank(['/mnt/data/streamIn1215.txt', '/mnt/data/streamIn1216.txt'])
