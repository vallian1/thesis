...
# 加载另一个脚本
mod = runpy.run_path(
    os.path.join(BASE_DIR, "experiment1", "experiment1_node_posterior.py")
)
...
tuned_csv_candidates = [
    ...
    'E:/MyLunWen/xduts-main/code/chapter3/experiment1/exp1_results_tuned/exp1_posterior_timeseries.csv',
]
tuned_csv = None
...
lines.append('')
lines.append('本实验用于验证第三章中地磁传感器自适应后验的正确性与稳定性，并评估其对在线跟踪性能的影响。')
lines.append('')
lines.append('## 主场景设置')
lines.append(f'- 仿真地磁传感器数量：72对、双车道。')
...
lines.append('')
lines.append('## 结果解读')
lines.append(f'- 在全部地磁传感器中统计，完整方法在退化区间内的检测率估计误差为 {full.pd_mae_degrade_interval:.4f}，优于规则基线的 {rule.pd_mae_degrade_interval:.4f}，也优于地磁传感器固定参数后验的 {glob.pd_mae_degrade_interval:.4f}。')
lines.append(f'- 在全部地磁传感器中统计，完整方法在退化区间内的多检率估计误差为 {full.pf_mae_degrade_interval:.4f}，低于规则基线的 {rule.pf_mae_degrade_interval:.4f}。')
lines.append(f'- 跟踪层面，完整方法的 IDSW={int(full.IDSW)}、Frag={int(full.Frag)}、FalseDelete={int(full.FalseDelete)}，均优于规则基线；相对地磁传感器固定参数后验，完整方法的 FalseDelete 也进一步下降。')
lines.append(f'- 创新方差与地磁传感器可靠性的相关系数为 {full.innovation_variance_reliability_corr:.3f}，呈显著负相关，支持将 R_b 与 r_b 绑定的设计。')
lines.append('')
